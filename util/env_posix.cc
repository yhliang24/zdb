...
class HMComRamdomAccessFile : public RandomAccessFile {  //when compaction,we can read the entire file at once
  ...
  public:
    HMComRamdomAccessFile(const std::string &fname, HMManager* hm_manager,const char *buf_file)
      ...
      file_size = hm_manager_->get_table_size(filenum);
      //buf_ = new char[ldb->size];
      int ret=posix_memalign((void **)&buf_,MEMALIGN_SIZE,file_size);
      if(ret!=0){
      ...
      st=Status::OK();
      if(buf_file != NULL){
          memcpy(buf_,buf_file,file_size);
      }
      else{
        r = hm_manager_->hm_read(filenum, buf_, file_size, 0);
        if(r<0){
          st= IOError(filename_, errno);
        }
      }
    }
...
class HMWritableFile : public WritableFile {    //hm write file except L0 level
  ...
  public:
      ...
      : fname_(fname),total_size_(0), hm_manager_(hm_manager),level_(level)
    {
      ...
      /*
	#ifdef NOT_RESIZE
      uint64_t size=(64*1024*1024); //todo
      #else*/
	...
	/*
    #ifdef NOT_RESIZE
	if(total_size_ + data.size() > 256 * 1024 * 1024 && total_size_ <= 256 * 1024 * 1024){
		printf("resize 512\n");
		size = 512 * 1024 * 1024;
	}
	else if (total_size_ + data.size() > 128 * 1024 * 1024 && total_size_ <= 128 * 1024 * 1024){
		printf("resize 256\n");
		size = 256 * 1024 * 1024;
	}
	else if(total_size_ + data.size() > 64 * 1024 * 1024 && total_size_ <= 64 * 1024 * 1024){
		printf("resize 128\n");
		size = 128 * 1024 * 1024;
	}
	if(size != 0){
		char* tmp;
	        int ret=posix_memalign((void **)&tmp,MEMALIGN_SIZE,size);
	        if(ret!=0){
	          printf("error:%d posix_memalign falid!\n",ret);
      		}
		memcpy(tmp, buf_, total_size_);
		free(buf_);
		buf_ = tmp;
	}
         #endif
       */
      if (total_size_ + data.size() > 256 * 1024 * 1024){
		    printf("resize 256\n");
		    size = 515 * 1024 * 1024;
	    }
      if(size != 0){
    ...
    virtual void set_guard(const std::string& guard_key_pass_down) {
    if(guard_key_pass_down.size() != 0) {
      guard_key = guard_key_pass_down;
    }
    return;
    };

  virtual void set_level(int level) {level_ = level; return;};
...
class PosixEnv : public Env {
 public:
  ...
  virtual Status NewRandomAccessFile(const std::string& fname,
                                     RandomAccessFile** result, const char *buf_file, bool fill_gd_cache=false) {
    *result = NULL;
    Status s;
  ...
  virtual Status NewWritableFile(const std::string& fname,
    ...
    if(isSSTableName(fname)){
        *result = new HMWritableFile(fname, Singleton::Gethmmanager(),0);
        return Status::OK();
    }
