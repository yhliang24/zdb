...
public:
    ...
    NewRandomAccessFile(const std::string &fname,
                        RandomAccessFile **result, const char *buf_file = NULL, bool fill_gd_cache=false) = 0;
...
public:
    ...
    Sync() = 0;
    virtual void 
    set_guard(const std::string& guard_key_pass_down) {return;};
    virtual void 
    set_level(int level) {return;};
