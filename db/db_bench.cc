    ...
    namespace {
        ...
        class Stats
        {
          ...
          public:
            ...
            double start_;
            Stats()
                : start_(), finish_(), seconds_(), done_(), next_report_(), bytes_(), last_op_finish_(), hist_(),
                  message_()
            {
                Start();
            }
        ...
        struct SharedState {
            ...
            SharedState() : mu(), cv(&mu), total(), num_initialized(), num_done(), start()
            {
            }
        };
        ...
        struct ThreadState {
            ...
            ThreadState(int index) : tid(index), rand(1000 + index), stats(), shared()
            {
            }
      ...
      public:
        Benchmark()
            : cache_(FLAGS_cache_size >= 0 ? NewLRUCache(FLAGS_cache_size) : NULL),
              filter_policy_(FLAGS_bloom_bits >= 0 ? NewBloomFilterPolicy(FLAGS_bloom_bits) : NULL), db_(NULL),
              num_(FLAGS_num), value_size_(FLAGS_value_size), entries_per_batch_(1), write_options_(),
              reads_(FLAGS_reads < 0 ? FLAGS_num : FLAGS_reads), hm_manager_(Singleton::Gethmmanager()),
              heap_counter_(0)
        {
        ...
        unsigned long long print_splitup(int tid)
        {
            ...
            printf("YCSB splitup: R = %llu, D = %llu, I = %llu, U = %llu, S = %llu\n", result.ycsb_r, result.ycsb_d,
                   result.ycsb_i, result.ycsb_u, result.ycsb_s);
            printf("LevelDB/WiscKey splitup: P = %llu, G = %llu, D = %llu, ItSeek = %llu, ItNext = %llu\n", result.kv_p,
                   result.kv_g, result.kv_d, result.kv_itseek, result.kv_itnext);
        ...
        void parse_trace(const char *file, int tid)
        {
            ...
            if (tid >= num_trace_files) {
                corresponding_file =
                    file_names[num_trace_files - 1];  // Take the last file if number of files is lesser
            } else {
                corresponding_file = file_names[tid];
            }
            printf("Thread %d: Parsing trace ...\n", tid);
            trace_ops[tid] =
                (struct trace_operation_t *)mmap(NULL, MAX_TRACE_OPS * sizeof(struct trace_operation_t),
...
#define envinput(var, type)                         \
    {                                               \
        assert(getenv(#var));                       \
        int ret = sscanf(getenv(#var), type, &var); \
        assert(ret == 1);                           \
    }
#define envstrinput(var) strcpy(var, getenv(#var))

        void print_all_info(uint64_t mark)
        {
            std::string property;
            std::string tmp;
            // printf("mark %d\n", mark);
            hm_manager_->get_all_info(mark);
            property.append("Guard\n");
            for (int level = 0; level < 7; level++) {
                db_->GetProperty("leveldb.guard-details-at-level" + NumberToString(level), &tmp);
                property.append(tmp);
            }
            property.append("\nsentinel\n");
            for (int level = 0; level < 7; level++) {
                db_->GetProperty("leveldb.sentinel-details-at-level" + NumberToString(level), &tmp);
                property.append(tmp);
            }
            printf("i=%ld\n", mark);
        ...
        {
            int tid = thread->tid;
            char load_trace_file[1000] = "/home/yjwang/ycsbtrace/80GB_l.trace";
            char run_trace_file[1000] = "/home/yjwang/ycsbtrace/80GB_la.trace";

            // envstrinput(trace_file);

            parse_trace(load_trace_file, tid);
            ...
            unsigned long long total_ops = 0;
            double start, end, curr_time, last_time, secs;

            printf("Thread %d: loading trace ...\n", tid);
            ...
            while (curop->cmd) {
                ...
                if ((total_ops + 1) % 100 == 0) {
                    hm_manager_->print_space_amplication(total_ops + 1);
                }

                if ((total_ops + 1) % 1000000 == 0) {
                    curr_time = Env::Default()->NowMicros();
                    double elapsed = curr_time - last_time;
                    last_time = curr_time;
                    MyLog2("i=%d, every 1000000 ops(1G), time=%.2f s run_time:%.2f s\n", total_ops + 1,
                           elapsed / 1000000.0, (curr_time - start) / 1000000.0);
                    MyLog("i=%d, every 1000000 ops(1G), time=%.2f s run_time:%.2f s\n", total_ops + 1,
                          elapsed / 1000000.0, (curr_time - start) / 1000000.0);
                    printf("i=%lld, every 1000000 ops(1G), time=%.2f s run_time:%.2f s\n", total_ops + 1,
                           elapsed / 1000000.0, (curr_time - start) / 1000000.0);
                    print_all_info(total_ops + 1);
                }
            }
            hm_manager_->get_zone_info();
            PrintStats("leveldb.stats");
            fprintf(stderr, "\r");
            end = Env::Default()->NowMicros();
            secs = (end - start) / 1000000.0;
            ...
            MyLog("start trace\n");
            // run trace
            parse_trace(run_trace_file, tid);

            hm_manager_->print_read_log();
            results[tid].clear();
            curop = trace_ops[tid];
            unsigned long long run_ops = 0;
            ...
            while (curop->cmd) {
                ...
                if (run_ops % 1000 == 0) {
                    ...
                    double elapsed = curr_time - last_time;
                    last_time = curr_time;
                    MyLog2("i=%d, every 1000 ops, time=%.2f s run_time:%.2f s\n", run_ops, elapsed / 1000000.0,
                           (curr_time - start) / 1000000.0);
                    MyLog("i=%d, every 1000 ops, time=%.2f s run_time:%.2f s\n", run_ops, elapsed / 1000000.0,
                          (curr_time - start) / 1000000.0);
                    printf("i=%lld, every 1000 ops, time=%.2f s run_time:%.2f s\n", run_ops, elapsed / 1000000.0,
                           (curr_time - start) / 1000000.0);
                    print_all_info(run_ops);
                }
            }
            PrintStats("leveldb.stats");
            fprintf(stderr, "\r");
            ...
            result = results[tid];
            printf("\n\nThread %d: Done replaying %llu operations.\n", tid, run_ops);
            splitup_ops = print_splitup(tid);
            assert(splitup_ops == run_ops);
            printf("Thread %d: %lu of %llu operations succeeded.\n", tid, succeeded, run_ops);
            printf("Thread %d: Time taken = %0.3lf seconds\n", tid, secs);
            printf("Thread %d: Total data: YCSB = %0.6lf GB, HyperLevelDB = %0.6lf GB\n", tid,
                   double(result.ycsbdata) / 1024.0 / 1024.0 / 1024.0,
                   double(result.kvdata) / 1024.0 / 1024.0 / 1024.0);
            printf("Thread %d: Ops/s = %0.3lf Kops/s\n", tid, double(run_ops) / 1024.0 / secs);

            throughput = double(result.ycsbdata) / secs;
            printf("Thread %d: YCSB throughput = %0.6lf MB/s\n", tid, throughput / 1024.0 / 1024.0);
            throughput = double(result.kvdata) / secs;
            printf("Thread %d: HyperLevelDB throughput = %0.6lf MB/s\n", tid, throughput / 1024.0 / 1024.0);
        }

        void CheckDataBase(ThreadState *thread)
        {
            int tid = thread->tid;
            char load[1000] = "/ycsbtrace/10GB_l.loadtrace";
            char Check[1000] = "/ycsbtrace/10GB_l.checktrace";
            // envstrinput(trace_file);

            std::ifstream infile;
            infile.open(load, std::ios::in);
            ...
            fprintf(stderr, "\nCompleted 0 ops");
            uint64_t succeeded = 0;
            while (getline(infile, key_value)) {
                key.clear();
                value.clear();
                pos = key_value.find(" ");
                key = key_value.substr(0, pos);
                value = key_value.substr(pos + 1, key_value.size());
                // std::cout << key << " " << value << std::endl;
                Status status = db_->Put(woptions, Slice(key), Slice(value));
                if (status.ok()) {
                    succeeded++;
                }
                thread->stats.FinishedSingleOp();
            ...
            while (getline(infile, key_value)) {
                ...
                std::string rvalue;
                // std::cout << key << " " << value << std::endl;
                if (db_->Get(options, Slice(key), &rvalue).ok()) {
                    if (value.compare(rvalue) != 0) {
                        printf("ERROR !! read %d Key %s is not match in database:%s %s\n", total, key.c_str(),
                               value.c_str(), rvalue.c_str());
                        assert(0);
                    }
                    found++;
                } else {
        ...
        void Run()
        {
            ...
            while (benchmarks != NULL) {
                ...
                } else if (name == Slice("checkdatabase")) {
                    fresh_db = true;
                    //        entries_per_batch_ = 1000;
                    method = &Benchmark::CheckDataBase;
      ...
      private:
        ...
        void SnappyCompress(ThreadState *thread)
        {
            ...
            if (!ok) {
                ...
                char buf[100];
                snprintf(buf, sizeof(buf), "(output: %.1f%%)", (produced * 100.0) / bytes);
        ...
        void DoWrite(ThreadState *thread, bool seq)
        {
            ...
            for (int i = 0; i < num_; i += entries_per_batch_) {
                // if((i + 1) % 2500000 == 0) {
                //  printf("print stale when loading %d\n", i + 1);
                //  db_->PrintFilesSizeByGuard();
                //}

	      double prev_time = Env::Default()->NowMicros();
                batch.Clear();
                for (int j = 0; j < entries_per_batch_; j++) {
       ...
       printf("gap %lf\n", Env::Default()->NowMicros() - prev_time);
                if ((i + 1) % 100 == 0) {
                    hm_manager_->print_space_amplication(i + 1);
                }
                ...
                if ((i + 1) % 250000 == 0 || (i + 1) == num_) {
                    ...
                    last_time = now;
                    last_bytes = bytes;
                    MyLog2("i=%d, every 250000 ops(1G), speed=%2.1lfMB/s time=%.2f s run_time:%.2f s size:%.2f MB\n",
                           i + 1, (ebytes / 1048576.0) / elapsed, elapsed, mytime, bytes / 1048576.0);
                    MyLog("i=%d, every 250000 ops(1G), speed=%2.1lfMB/s time=%.2f s run_time:%.2f s size:%.2f MB\n",
                          i + 1, (ebytes / 1048576.0) / elapsed, elapsed, mytime, bytes / 1048576.0);
                    printf("i=%d, every 250000 ops(1G), speed=%2.1lfMB/s time=%.2f s run_time:%.2f s size:%.2f MB\n",
                           i + 1, (ebytes / 1048576.0) / elapsed, elapsed, mytime, bytes / 1048576.0);
                    print_all_info(i + 1);
                    /*
                      std::string property;
        ...
        void ReadWhileWriting(ThreadState *thread)
        {
            if (thread->tid > 0) {
                ...
                micros(after_g);
                print_timer_info(
                    "ReadwhileWriting() method :: Total time took to read all entries while writing other entries",
                    after_g, before_g);
                // Do not count any of the preceding work/delay in stats.
                thread->stats.Start();
            }
