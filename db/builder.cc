...
namespace leveldb {
    ...
    void FinishFileCompletion(Status s, FileMetaData meta, WritableFile *file, TableCache *table_cache, Env *env,
                              std::string fname)
    {
        if (s.ok()) {
            // currently only level 0 will invoke this function
            file->set_level(0);
            // level 0 has no guard, so no need to set_guard
            s = file->Sync();
        }
        if (s.ok()) {
            s = file->Close();
        ...
        const char *buf_file = file->Getbuf();
        Iterator *iter = table_cache->NewIterator(options, meta.number, meta.file_size, NULL, buf_file);
        s = iter->status();
        delete iter;
#endif
    ...
    void AddFilterString(FileLevelFilterBuilder *file_level_filter_builder, int n,
                         std::vector<std::string *> *filter_list, const FilterPolicy *filter_policy,
    ...
    Status BuildLevel0Tables(const std::string &dbname, Env *env, const Options &options, TableCache *table_cache,
                             Iterator *iter, std::vector<FileMetaData> *meta_list,
                             std::vector<std::string *> *filter_list, VersionSet *versions_,
                             std::set<uint64_t> *pending_outputs_, std::vector<GuardMetaData *> complete_guards_,
                             port::Mutex *mutex_, uint64_t *reserved_file_numbers,
                        ...
                        if (count > 0) {
                            ...
                            if (s.ok()) {
                                ...
                                AddFilterString(file_level_filter_builder, index, filter_list, filter_policy,
                                                meta.number);
                                table_cache->SetFileMetaDataMap(meta.number, meta.file_size, meta.smallest,
                                                                meta.largest);
                            }
                            delete builder;
                            count = 0;
                            ...
                            table_nums.push_back(((*meta_list)[meta_list->size() - 1]).number);
                            Singleton::Gethmmanager()->get_table_offsets(table_nums, table_offsets);
                            (*meta_list)[meta_list->size() - 1].disk_offset = table_offsets.at(0);
                        }
                        break;
                    }
                }
