...
namespace leveldb {
    ...
    struct DBImpl::Writer {
        ...
        explicit Writer(port::Mutex *mtx)
            : cv_(mtx), linked_(false), has_imm_(false), wake_me_when_head_(false), block_if_backup_in_progress_(true),
              prev_(NULL), next_(NULL), micros_(0), start_sequence_(0), end_sequence_(0), mem_(NULL), logfile_(), log_()
        {
        }
        ~Writer() throw()
        {
    ...
    struct DBImpl::CompactionState {
        ...
        struct Output {
            Output() : number(), file_size(), smallest(), largest(), disk_offset(-1)
            {
            }
            uint64_t number;
            uint64_t file_size;
        ...
        explicit CompactionState(Compaction *c)
            : compaction(c), smallest_snapshot(), outputs(), outfile(NULL), builder(NULL), total_bytes(0)
        {
        }
    ...
    template <class T, class V> static void ClipToRange(T *ptr, V minvalue, V maxvalue)
    {
    ...
    DBImpl::DBImpl(const Options &raw_options, const std::string &dbname)
        : env_(raw_options.env), internal_comparator_(raw_options.comparator),
          internal_filter_policy_(raw_options.filter_policy), filter_policy_(raw_options.filter_policy),
          options_(SanitizeOptions(dbname, &internal_comparator_, &internal_filter_policy_, raw_options)),
          owns_info_log_(options_.info_log != raw_options.info_log),
          owns_cache_(options_.block_cache != raw_options.block_cache), dbname_(dbname), table_cache_(), db_lock_(NULL),
          mutex_(), shutting_down_(NULL), mem_(new MemTable(internal_comparator_)), imm_(NULL), has_imm_(), logfile_(),
          logfile_number_(0), log_(), seed_(0), writers_mutex_(), writers_upper_(0), writers_tail_(NULL), snapshots_(),
          pending_outputs_(), allow_background_activity_(false), num_bg_threads_(0), bg_fg_cv_(&mutex_),
          bg_compaction_cv_(&mutex_), bg_memtable_cv_(&mutex_), bg_log_cv_(&mutex_), bg_log_occupied_(false),
          manual_compaction_(NULL),
          manual_garbage_cutoff_(raw_options.manual_garbage_collection ? SequenceNumber(0) : kMaxSequenceNumber),
          replay_iters_(), straight_reads_(0), versions_(), backup_cv_(&writers_mutex_), backup_in_progress_(),
          backup_waiters_(0), backup_waiter_has_it_(false), backup_deferred_delete_(), bg_error_(),
          num_bg_compaction_threads_(1), hm_manager_(Singleton::Gethmmanager())
    {
        mutex_.Lock();
        mem_->Ref();
        ...
        timer = new Timer();
        versions_ = new VersionSet(dbname_, &options_, table_cache_, &internal_comparator_, timer);
    ...
    void DBImpl::DeleteObsoleteFiles()
    {
        ...
        env_->GetChildren(dbname_, &filenames);  // Ignoring errors on purpose
        {
            ...
            std::lock_guard<std::mutex> lk(hm_manager_->mtx_);
            #else
            std::shared_lock<std::shared_mutex> lock(hm_manager_->mutex_);
            #endif
            std::map<uint64_t, struct Ldbfile *> *table;
            hm_manager_->get_table(&table);
            std::map<uint64_t, struct Ldbfile *>::iterator it;
            char buf[100];
            for (it = table->begin(); it != table->end(); it++) {
                snprintf(buf, sizeof(buf), "%06lu.sst", it->first);
    ...
    Status DBImpl::Recover(VersionEdit *edit)
    {
        ...
        if (!env_->FileExists(CurrentFileName(dbname_))) {
            if (options_.create_if_missing) {
                printf("new db %s\n", CurrentFileName(dbname_).c_str());
                s = NewDB();
                hm_manager_->reset_zone();
                if (!s.ok()) {
                    return s;
                }
            } else {
        ...
        if (s.ok()) {
            ...
            const uint64_t min_log =
                versions_->LogNumber();  // This will be populated by versions_->Recover() from manifest file
    ...
    Status DBImpl::RecoverLogFile(uint64_t log_number, VersionEdit *edit, SequenceNumber *max_sequence)
    {
        struct LogReporter : public log::Reader::Reporter {
            LogReporter() : env(), info_log(), fname(), status()
            {
            }
            Env *env;
            Logger *info_log;
        ...
        while (reader.ReadRecord(&record, &scratch) && status.ok()) {
            ...
            status = WriteBatchInternal::InsertIntoVersion(&batch, mem, versions_->current());
    ...
    Status DBImpl::WriteLevel0TableGuards(MemTable *mem, VersionEdit *edit, Version *base,
                                          std::vector<uint64_t> &numbers,
                                          ...
                                          std::vector<std::string *> *file_level_filters)
    {
        ...
        Status s;
        {
            ...
            s = BuildLevel0Tables(dbname_, env_, options_, table_cache_, iter, &meta_list, file_level_filters,
                                  versions_, &pending_outputs_, guards_, &mutex_, reserved_file_numbers,
                                  file_level_filter_builder);
        ...
        for (unsigned i = 0; i < meta_list.size(); i++) {
            ...
            if (s.ok() && meta.file_size > 0) {
                ...
                edit->AddFile(level, meta.number, meta.file_size, meta.smallest, meta.largest, meta.disk_offset);
                numbers.push_back(meta.number);
                total_file_size += meta.file_size;
            }
        }

        // Adding the remaining reserved but unused file numbers so that they can be removed from the pending_outputs
        // set
    ...
    void DBImpl::CompactMemTableThread()
    {
        ...
        while (!shutting_down_.Acquire_Load()) {
            ...
            Status s =
                WriteLevel0TableGuards(imm_, &edit, base, numbers, &file_level_filter_builder, &file_level_filters);
            ...
            if (s.ok()) {
                ...
                edit.SetStaleRatio(-1.0);
                start_timer(CMT_LOG_AND_APPLY);
                s = versions_->LogAndApply(&edit, &mutex_, &bg_log_cv_, &bg_log_occupied_, numbers, file_level_filters,
                                           1);
            ...
            if (!shutting_down_.Acquire_Load() && !s.ok()) {
                ...
                Log(options_.info_log, "Waiting after memtable compaction error: %s", s.ToString().c_str());
                mutex_.Unlock();
    ...
    void DBImpl::TEST_CompactAllLevels()
    {
        while (!versions_->IsAllLevelsCompacted()) {
            bool force_compact;
            bool is_rewrite;
            bool guard_seek_compaction;
            int level = versions_->PickCompactionLevel(levels_locked_, false, &is_rewrite, &guard_seek_compaction);
    ...
    void DBImpl::TEST_ComapactFilesToSingleLevel()
    {
        while (true) {
            ...
            int num_active_levels =
                versions_->current()->GetNumLevelsWithFiles(&min_level, &max_level, &max_level_compaction_score);
    ...
    void DBImpl::CompactLevelThread()
    {
        ...
        while (!shutting_down_.Acquire_Load()) {
            ...
            for (int level = 0; level < 7; level++) {
                property.append(versions_->GuardDetailsAtLevel(level));
            }
            int flag = 0;
            if (property.size() < 1024000 - 4)
                MyLog2("\n%s\n", property.c_str());
            else {
                printf("\n%s\n", property.c_str());
                flag = 1;
            ...
            property.clear();
            for (int level = 0; level < 7; level++) {
                property.append(versions_->SentinelDetailsAtLevel(level));
            }
            tmp.clear();
            char buf[200];
            ...
            if (s.ok()) {
                ...
                // chew up resources for failed compactions for the duration of
                // the problem.
                Log(options_.info_log, "Waiting after background compaction error: %s", s.ToString().c_str());
                mutex_.Unlock();
    ...
    Status DBImpl::BackgroundCompactionGuards(FileLevelFilterBuilder *file_level_filter_builder)
    {
        ...
        PiggyMain *pm = new PiggyMain();
        bool is_rewrite = false;
        bool guard_seek_compaction = false;
        if (is_manual) {
            ...
            unsigned level = versions_->PickCompactionLevel(levels_locked_, straight_reads_ > kStraightReads,
                                                            &is_rewrite, &guard_seek_compaction);

            record_timer(BGC_PICK_COMPACTION_LEVEL);

            start_timer(BGC_PICK_COMPACTION);
            if (level != config::kNumLevels) {
                c = versions_->PickCompactionForGuards(versions_->current(), level,
                                                       &complete_guards_used_in_bg_compaction,
                                                       &complete_guards_used_in_bg_compaction_old_level,
                                                       &full_guard_added_in_compaction, pm, guard_seek_compaction);
            }
        ...
        if (c == NULL) {
            ...
            // Add the guard information in current version to edit (primarily to add the new guards in
            // complete_guards_)
            std::set<int> level_to_load_from_complete_guards;
            // NOTE: The complete guards are copied to the guards only during background compaction and not during
            // manual compaction. It is debatable and needs code change while picking compaction if this needs to be
            // enabled.
            if (!is_manual) {
            ...
            compact->compaction->edit()->SetComLevel(c->level());
            if (!c->is_horizontal_compaction) {
                int compaction_level = compact->compaction->level();
                printf("compact level %d complete guard involved in compaction %ld\n", c->level(),
                       complete_guards_used_in_bg_compaction_old_level.size());
                printf("compact level %d complete guard involved in compaction %ld\n", c->level() + 1,
                       complete_guards_used_in_bg_compaction.size());
                for (int j = 0; j < complete_guards_used_in_bg_compaction_old_level.size(); j++) {
                    compact->compaction->edit()->AddGuardFromExisting(
                        c->level(), complete_guards_used_in_bg_compaction_old_level[j]);
                }
                for (int j = 0; j < complete_guards_used_in_bg_compaction.size(); j++) {
                    compact->compaction->edit()->AddGuardFromExisting(c->level() + 1,
                                                                      complete_guards_used_in_bg_compaction[j]);
                }
            } else {
                int compaction_level = compact->compaction->level();
                printf("compact level %d complete guard involved in compaction %ld\n", c->level(),
                       complete_guards_used_in_bg_compaction.size());
                for (int j = 0; j < complete_guards_used_in_bg_compaction.size(); j++) {
                    compact->compaction->edit()->AddGuardFromExisting(c->level(),
                                                                      complete_guards_used_in_bg_compaction[j]);
                }
                for (int j = 0; j < full_guard_added_in_compaction.size(); j++) {
                    compact->compaction->edit()->AddFullGuardAddedInCompaction(
                        c->level(), full_guard_added_in_compaction[j]);  // only level() should compact sst
                }
                compact->compaction->edit()->SetIsRewrite(c->level(), is_rewrite);
                pm->Debug();
                compact->compaction->edit()->SetPM(pm);
                delete pm;
            }
    ...
    Status DBImpl::FinishCompactionOutputFile(CompactionState *compact, Iterator *input,
                                              ...
                                              std::vector<std::string *> *file_level_filters,
                                              const std::string &guard_key_pass_down, int level_written_to)
    {
        assert(compact != NULL);
        assert(compact->outfile != NULL);
        assert(compact->builder != NULL);
        ...
        if (s.ok()) {
            compact->outfile->set_level(level_written_to);
            compact->outfile->set_guard(guard_key_pass_down);
        ...
        std::vector<int64_t> table_nums;
        std::vector<int64_t> table_offsets;
        table_nums.push_back(output_number);
        hm_manager_->get_table_offsets(table_nums, table_offsets);
        compact->current_output()->disk_offset = table_offsets.at(0);
        ...
        if (s.ok() && current_entries > 0) {
#if Verify_Table
            ReadOptions options;
            options.fill_geardb_cache =
                true;  // this is geardb design, by default, disable it (read whole table == false)
            const char *buf_file = compact->outfile->Getbuf();
            Iterator *iter = table_cache_->NewIterator(options, output_number, current_bytes, NULL, buf_file);
            s = iter->status();
            delete iter;
#endif
            ...
            if (s.ok()) {
                Log(options_.info_log, "Generated table #%llu: %lld keys, %lld bytes",
                    (unsigned long long)output_number, (unsigned long long)current_entries,
    ...
    {
        mutex_.AssertHeld();
        Log(options_.info_log, "Compacted %lu@%d + %lu@%d files => %lld bytes", compact->compaction->num_input_files(0),
            compact->compaction->level(), compact->compaction->num_input_files(1), compact->compaction->level() + 1,
            static_cast<long long>(compact->total_bytes));

        MyLog("Compacted %lu@%d + %lu@%d files => %lld bytes", compact->compaction->num_input_files(0),
              compact->compaction->level(), compact->compaction->num_input_files(1), compact->compaction->level() + 1,
              static_cast<long long>(compact->total_bytes));

        printf("Compacted %lu@%d + %lu@%d files => %lld bytes", compact->compaction->num_input_files(0),
              compact->compaction->level(), compact->compaction->num_input_files(1), compact->compaction->level() + 1,
              static_cast<long long>(compact->total_bytes));

        MyLog2("Compacted %lu@%d + %lu@%d files => %lld bytes", compact->compaction->num_input_files(0),
               compact->compaction->level(), compact->compaction->num_input_files(1), compact->compaction->level() + 1,
               static_cast<long long>(compact->total_bytes));

        // calculate the duplicate ratio of compacting level
        uint64_t before_size = 0;
        for (int which = 0; which < 2; which++) {
            for (size_t i = 0; i < compact->compaction->num_input_files(which); i++) {
                if (which == 0) {
                    before_size += compact->compaction->input(which, i)->file_size;
                } else {
                    before_size += compact->compaction->input(which, i)->file_size;
                }
            }
        }
        double ratio = static_cast<double>(compact->total_bytes) / static_cast<double>(before_size);
        if (ratio > 0.0 && ratio < 1.0)
            compact->compaction->edit()->SetStaleRatio(ratio);
        else {
            compact->compaction->edit()->SetStaleRatio(-1.0);
        }
        ratio = ratio > 1.0 ? 1.0 : ratio;  // sometimes because of metadata changed by different sst num, metadata size
                                            // can be increased. Thus, it may enlarge the size
        for (int i = 0; i < config::kNumLevels; i++) {
            if (i == compact->compaction->level()) {
                compact->compaction->edit()->SetDupRatio(i, ratio);
                compact->compaction->edit()->SetBeforeSize(i, static_cast<double>(before_size) / 1024.0 / 1024.0);
                compact->compaction->edit()->SetAfterSize(i,
                                                          static_cast<double>(compact->total_bytes) / 1024.0 / 1024.0);
                printf("level %d dupratio %lf\n", i, ratio);
                printf("level %d beforesize %lf\n", i, static_cast<double>(before_size) / 1024.0 / 1024.0);
                printf("level %d aftersize %lf\n", i, static_cast<double>(compact->total_bytes) / 1024.0 / 1024.0);
            } else {
                compact->compaction->edit()->SetDupRatio(i, 0.0);
                compact->compaction->edit()->SetBeforeSize(i, 0.0);
                compact->compaction->edit()->SetAfterSize(i, 0.0);
            }
        }
        ...
        for (size_t i = 0; i < compact->outputs.size(); i++) {
            const CompactionState::Output &out = compact->outputs[i];
            compact->compaction->edit()->AddFile(level_to_add_new_files, out.number, out.file_size, out.smallest,
                                                 out.largest, out.disk_offset);
        }
        return versions_->LogAndApply(compact->compaction->edit(), &mutex_, &bg_log_cv_, &bg_log_occupied_,
                                      file_numbers, file_level_filters, 0);
    }
                                          ...
                                          FileLevelFilterBuilder *file_level_filter_builder)
    {
        ...
        Log(options_.info_log, "Compacting %lu@%d (%lfMB) + %lu@%d files (%lfMB) for guards in a level",
            compact->compaction->num_input_files(0), compact->compaction->level(),
            current_level_input_data_size / 1024.0 / 1024.0, compact->compaction->num_input_files(1),
            compact->compaction->level() + 1, next_level_input_data_size / 1024.0 / 1024.0);

        Log(options_.info_log, (versions_->GetCurrentVersionState()).c_str());

        MyLog("current zone num %ld\n", hm_manager_->get_zone_num());
        MyLog("Compacting %lu@%d (%lfMB) + %lu@%d files (%lfMB) for guards in a level",
              compact->compaction->num_input_files(0), compact->compaction->level(),
              current_level_input_data_size / 1024.0 / 1024.0, compact->compaction->num_input_files(1),
              compact->compaction->level() + 1, next_level_input_data_size / 1024.0 / 1024.0);

        printf("Compacting %lu@%d (%lfMB) + %lu@%d files (%lfMB) for guards in a level",
              compact->compaction->num_input_files(0), compact->compaction->level(),
              current_level_input_data_size / 1024.0 / 1024.0, compact->compaction->num_input_files(1),
              compact->compaction->level() + 1, next_level_input_data_size / 1024.0 / 1024.0);
        ...
        for (; input->Valid() && !shutting_down_.Acquire_Load();) {
            ...
            if (!ParseInternalKey(key, &ikey)) {
                ...
                if (!has_current_key || user_comparator()->Compare(ikey.user_key, current_key.user_key) != 0) {
                    if (has_current_key && compact->builder &&
                        compact->builder->FileSize() >= compact->compaction->MinOutputFileSize() &&
                        compact->compaction->CrossesBoundary(
                            current_key, ikey, &boundary_hint)) {  // crossboundary is useless in pebblesdb
                        // LYH
                        std::string guard_key_pass_down;
                        if (num_guards != 0) {
                            if (user_comparator()->Compare((compact->current_output()->largest).user_key(),
                                                           guards[0]->guard_key.user_key()) >= 0) {
                                // get the guard num
                                assert(current_guard >= 0);
                                guard_key_pass_down = guards[current_guard - 1]->guard_key.user_key().ToString();
                                // printf("com1 %s\n", guard_key_pass_down.c_str());
                            }
                        }
                        ...
                        status = FinishCompactionOutputFile(compact, input, file_level_filter_builder, &file_numbers,
                                                            &file_level_filters, guard_key_pass_down, level_written_to);
                        index = 0;
                ...
                // it.
                if (drop && last_sequence_for_key == kMaxSequenceNumber && ikey.sequence >= manual_garbage_cutoff_) {
                    drop = false;
                }
            ...
            if (!drop) {
                ...
                if (current_guard < guards.size() &&
                    ...
                    while (temp < guards.size() &&
                           user_comparator()->Compare(current_ikey.user_key(), guards[temp]->guard_key.user_key()) >=
                               0) {
                        temp++;
                    }
                ...
                int64_t max_left =
                    int64_t(hm_manager_->open_zone_max_left(level_written_to)) - 2 * 1024 * 1024 - 512 * 1024;
                int64_t allow_left = 2 * 1024 * 1024;  // avoid too small
                max_left = std::max(max_left, allow_left);
                assert(max_left >= 0);

                uint64_t max_output_file_size = std::min(uint64_t(max_left), compact->compaction->MaxOutputFileSize());
                ...
                if (compact->builder->FileSize() >=
                    ...
                    start_timer(BGC_FINISH_COMPACTION_OUTPUT_FILE);
                    // LYH
                    printf("min_output_file_size %ld %ld\n", max_output_file_size, max_left);
                    std::string guard_key_pass_down;
                    if (num_guards != 0) {
                        if (user_comparator()->Compare((compact->current_output()->largest).user_key(),
                                                       guards[0]->guard_key.user_key()) >= 0) {
    ...
    Iterator *DBImpl::NewInternalIterator(const ReadOptions &options, uint64_t number, SequenceNumber *latest_snapshot,
                                          uint32_t *seed, bool external_sync)
    {
        ...
        *latest_snapshot = versions_->LastSequence();
        ...
        Iterator *internal_iter = NewMergingIterator(&internal_comparator_, &list[0], list.size(), versions_);
    ...
    Status DBImpl::PrintFilesSizeByGuard()
    {
        MutexLock l(&mutex_);
        versions_->PrintGuardsStaleAndValid();
        Status s;
        return s;
    }
    ...
    void DBImpl::AllowGarbageCollectBeforeTimestamp(const std::string &timestamp)
    {
        ...
        } else if (GetVarint64(&ts_slice, &file) && GetVarint64(&ts_slice, &seqno)) {
            MutexLock l(&mutex_);
            if (manual_garbage_cutoff_ < seqno) {
                manual_garbage_cutoff_ = seqno;
            }
    ...
    bool DBImpl::ValidateTimestamp(const std::string &ts)
    {
        ...
        uint64_t seqno = 0;
        Slice ts_slice(ts);
        return ts == "all" || ts == "now" || (GetVarint64(&ts_slice, &file) && GetVarint64(&ts_slice, &seqno));
    }
    ...
    Status DBImpl::GetReplayIterator(const std::string &timestamp, ReplayIterator **iter)
    {
        *iter = NULL;
        Slice ts_slice(timestamp);
        uint64_t file = 0;
    ...
    void DBImpl::ReleaseReplayIterator(ReplayIterator *_iter)
    {
        ...
        for (std::list<ReplayIteratorImpl *>::iterator it = replay_iters_.begin(); it != replay_iters_.end(); ++it) {
            if (*it == iter) {
    ...
    void DBImpl::RecordReadSample(Slice key)
    {
        MutexLock l(&mutex_);
        ++straight_reads_;
        if (versions_->current()->RecordReadSample(key) || straight_reads_ > kStraightReads ||
            versions_->current()->RecordGuardReadSample(key)) {
            // if (versions_->current()->RecordReadSample(key)) {
            printf("signal background %ld\n", straight_reads_);
            bg_compaction_cv_.Signal();
        }
    }
    ...
    Status DBImpl::SequenceWriteBegin(Writer *w, WriteBatch *updates)
    {
...

        {
            ...
            if (enqueue_mem) {
                for (std::list<ReplayIteratorImpl *>::iterator it = replay_iters_.begin(); it != replay_iters_.end();
                     ++it) {
        ...
        if (s.ok()) {
            ...
            while (w->block_if_backup_in_progress_ && backup_in_progress_.Acquire_Load()) {
    ...
    void DBImpl::SequenceWriteEnd(Writer *w, WriteBatch *updates_with_guards, Status s)
    {
        ...
        if (s.ok() && updates_with_guards != NULL) {
            ...
            s = WriteBatchInternal::InsertIntoVersion(updates_with_guards, mem_, versions_->current());
...
/*
        if (w->micros_ > config::kL0_StopWritesTrigger) {
            //mutex_.Lock();
            //bg_fg_cv_.Wait();  // wait the background done the work
            //mutex_.Unlock();
//printf("num %ld pow sleep for %lf\n", w->micros_ , pow(w->micros_ , (w->micros_ * 1.0 / config::kL0_StopWritesTrigger)));
           //env_->SleepForMicroseconds(pow(w->micros_, (w->micros_ * 1.0 / config::kL0_StopWritesTrigger)) );
printf("num %ld pow sleep for %lf 10000 + pow(micro-stop , (w->micros_ - stop) \n", w->micros_ , 10000 + pow(w->micros_ - config::kL0_StopWritesTrigger , (w->micros_ * 1.0 - config::kL0_StopWritesTrigger)));
          env_->SleepForMicroseconds(std::min(1000000.0, 10000.0 + pow(w->micros_  - config::kL0_StopWritesTrigger , (w->micros_ - config::kL0_StopWritesTrigger))) );
// env_->SleepForMicroseconds(1000000.0);


            MyLog2("%d too many L0 files ...", w->micros_);
        } else if (w->micros_ > config::kL0_SlowdownWritesTrigger) {
            start_timer(SWE_SLEEP);
            env_->SleepForMicroseconds(1000);
            //env_->SleepForMicroseconds(w->micros_);
            record_timer(SWE_SLEEP);
        }
  */      
if (w->micros_ > config::kL0_StopWritesTrigger) {
	//printf("num %ld micro / 12 * 1000000 * 8  %lf\n", w->micros_, (w->micros_ / 12) * 1000000 * 8.0);
        env_->SleepForMicroseconds((w->micros_ / 12) * 1000000 * 8  );
}else if (w->micros_ > config::kL0_SlowdownWritesTrigger) {
          start_timer(SWE_SLEEP);
	//printf("num %ld 1000 * micro/8 %lf\n", w->micros_, 1000.0 );
          env_->SleepForMicroseconds(1000 * (w->micros_ / 8));
          record_timer(SWE_SLEEP);
        }
    ...
    bool DBImpl::GetProperty(const Slice &property, std::string *value)
    {
        ...
        if (in.starts_with("num-files-at-level")) {
            ...
            if (!ok || level >= config::kNumLevels) {
                ...
                char buf[100];
                snprintf(buf, sizeof(buf), "%d", num_guards);
                *value = buf;
                return true;
            }
            ...
            *value = versions_->current()->DebugString();
            ...
            snprintf(buf, sizeof(buf),
                     ...
                     "Count of filters: %lu \n",
                     filter_size_mb, filter_policy_->filter_count);
            value->append(buf);
            return true;
        }
    ...
    Status DB::Open(const Options &options, const std::string &dbname, DB **dbptr)
    {
        *dbptr = NULL;
        ...
        if (s.ok()) {
            ...
            s = options.env->NewConcurrentWritableFile(LogFileName(dbname, new_log_number), &lfile);
            if (s.ok()) {
