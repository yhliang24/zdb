...
namespace leveldb {
    ...
    void VersionEdit::Clear()
    {
        ...
        for (int i = 0; i < config::kNumLevels; i++) {
            is_rewrite[i] = false;
            dup_ratio[i] = 0.0;
            before_size[i] = 0.0;  // must be set
            after_size[i] = 0.0;   // must be set
        }

        piggy_level = -1;
        piggy_g = nullptr;
        piggy_full = false;
        piggy_file_metas.clear();
        piggy_cg.clear();

        compact_level = -1;
        compaction_remove_stale_ratio = -1.0;
    }
    ...
    void VersionEdit::EncodeTo(std::string *dst) const
    {
        ...
        for (size_t i = 0; i < new_files_.size(); i++) {
            ...
            MyLog("put fn %ld offset %lf (MB)", f.number, f.disk_offset / 1024.0 / 2.0);
            PutVarint64(dst, (uint64_t)f.disk_offset);  // add disk offset
            PutLengthPrefixedSlice(dst, f.smallest.Encode());
            PutLengthPrefixedSlice(dst, f.largest.Encode());
        }
        MyLog("\n");
        // Encode deleted guards
        for (DeletedGuardSet::const_iterator iter = deleted_guards_.begin(); iter != deleted_guards_.end(); ++iter) {
        ...
        for (size_t k = 0; k < config::kNumLevels; k++) {
            for (size_t i = 0; i < new_guards_[k].size(); i++) {
                ...
                PutVarint64(dst,
                            0 /* g.number_segments */);  // We don't write the file information to disk because it will
                                                         // be automatically retrieved from files_ in LogAndApply
        ...
        for (DeletedFileSet::const_iterator iter = deleted_sentinel_files_.begin();
             iter != deleted_sentinel_files_.end(); ++iter) {
            ...
            switch (tag) {
                ...
                case kNewFile:
                    if (GetLevel(&input, &level) && GetVarint64(&input, &f.number) &&
                        GetVarint64(&input, &f.file_size) && GetVarint64(&input, (uint64_t *)(&f.disk_offset)) &&
                        GetInternalKey(&input, &f.smallest) && GetInternalKey(&input, &f.largest)) {
                        MyLog2("extract fn %ld offset %lf (MB)", f.number, f.disk_offset / 1024.0 / 2.0);
                ...
                case kNewGuard:
                    if (GetLevel(&input, &g.level) && GetVarint64(&input, &g.number_segments) &&
                        GetInternalKey(&input, &g.guard_key)) {
                        /* Gather all the files inside the guard. */
                        g.files.clear();
                        if (g.number_segments > 0) {
                            assert(GetInternalKey(&input, &g.smallest) && GetInternalKey(&input, &g.largest));
