...
namespace leveldb {
    ...
    enum ThreadStatus { IDLE, ASSIGNED, BUSY, COMPLETED };
    enum SeekReturnStatus { UNINITIALIZED, INITIALIZED, ACKNOWLEDGED };
    ...
    extern int FindGuard(const InternalKeyComparator &icmp, const std::vector<GuardMetaData *> &guards,
                         const Slice &key);
    ...
    class Version
    {
      public:
        ...
        bool RecordGuardReadSample(Slice key);
        ...
        void GetOverlappingInputs(unsigned level,
                                  const InternalKey *begin,  // NULL means before all keys
                                  const InternalKey *end,    // NULL means after all keys
                                  std::vector<FileMetaData *> *inputs);

        void GetOverlappingInputsGuards(unsigned level,
                                        const InternalKey *begin,  // NULL means before all keys
                                        const InternalKey *end,    // NULL means after all keys
                                        std::vector<FileMetaData *> *inputs, std::vector<GuardMetaData *> *guard_inputs,
        ...
        bool OverlapInLevel(unsigned level, const Slice *smallest_user_key, const Slice *largest_user_key);

        bool OverlapInLevelGuards(unsigned level, const Slice *smallest_user_key, const Slice *largest_user_key);

        // Return the level at which we should place a new memtable compaction
        // result that covers the range [smallest_user_key,largest_user_key].
        int PickLevelForMemTableOutput(const Slice &smallest_user_key, const Slice &largest_user_key);

        int PickLevelForMemTableOutputGuards(const Slice &smallest_user_key, const Slice &largest_user_key);

        size_t NumFiles(unsigned level) const
        {
            return files_[level].size();
        }

        size_t NumGuards(unsigned level) const
        {
            return guards_[level].size();
        }
        ...
        size_t LevelGuardMaxSSTNum(int level)
        {
            assert(level < config::kNumLevels);
            int max_num = sentinel_files_[level].size();
            std::vector<GuardMetaData *> guards = guards_[level];
            for (unsigned i = 0; i < guards.size(); i++) {
                if (max_num < guards[i]->number_segments) {
                    max_num = guards[i]->number_segments;
                }
            }
            return max_num;
        }

        size_t NumSentinelFiles(unsigned level) const
        {
      ...
      private:
        ...
        int sentinel_files_allow_seeks[config::kNumLevels];
        ...
        int sen_file_to_compact_level_;

        GuardMetaData *guard_to_compact_;
        int guard_to_compact_level_;
        ...
        double last_level_wa_;

        double last_level6_bytes;

        double dup_ratio[config::kNumLevels];

        double before_size[config::kNumLevels];  // MB
        double after_size[config::kNumLevels];   // MB

        int no_need_to_compact[config::kNumLevels];

        std::vector<double> guard_compaction_scores_[config::kNumLevels];

        GuardMetaData *largest_guards[config::kNumLevels];

        int64_t compact_level;

        std::deque<double> last_n_stale_ratio;

        // To hold the compaction score of sentinel files in each level
        double sentinel_compaction_scores_[config::kNumLevels];

        explicit Version(VersionSet *vset)
            : vset_(vset), next_(this), prev_(this), refs_(0), file_to_compact_(NULL), file_to_compact_level_(-1)
        {
            for (unsigned i = 0; i < config::kNumLevels; ++i) {
                compaction_scores_[i] = -1;
                num_complete_guards_[i] = 0;
                no_need_to_compact[i] = 0;
                dup_ratio[i] = 1.0;
                before_size[i] = 0.0;
                after_size[i] = 0.0;
                sentinel_files_allow_seeks[i] = G_ALLOW_SEEK;
            }
            compact_level = -1;
    ...
    class VersionSet
    {
      public:
        VersionSet(const std::string &dbname, const Options *options, TableCache *table_cache,
                   const InternalKeyComparator *, Timer *timer);
        ~VersionSet();
        ...
        static void ConcurrentSeekWrapper(void *vset)
        {
            ...
            int index =
                vset_->seek_pthread_index_map_[thread_id];  // Get the static mapping of thread id to integer index
            ...
            while (vset_->stop_seek_threads_ == 0) {
                ...
                int group_index =
                    vset_->seek_assigned_group_id_[index];  // This is used to coordinate between a set of threads which
                                                            // belong to the same parallel call
...
#    ifdef SEEK_TWO_WAY_SIGNAL
                ...
                if (last_thread_to_complete) {
                    while (vset_->seek_group_return_status_[group_index] !=
                           ACKNOWLEDGED) {  // Kind of busy waiting here.
                        vset_->seek_group_threads_cv_[group_index]
...
#    endif
        ...
        // Returns the index of the thread which is idle. Returns -1 if no thread is idle.
        int GetReadIdleThreadIndex(pthread_t current_thread) {
            int index = -1;
            get_mutex_.Lock();
        ...
        Status LogAndApply(VersionEdit *edit, port::Mutex *mu, port::CondVar *cv, bool *wt,
                           std::vector<uint64_t> file_numbers, std::vector<std::string *> file_level_fitlers, int mtc)
        ...
        unsigned PickCompactionLevel(bool *locked, bool seek_driven, bool *is_rewrite,
        ...
        // in a level and assign them to proper guards
        Compaction *
        PickCompactionForGuards(Version *v, unsigned level,
                                ...
                                bool seek_compaction = false, bool guard_seek_compaction = false);

        // Return a compaction object for compacting the range [begin,end] in
        // the specified level.  Returns NULL if there is nothing in that
        // level that overlaps the specified range.  Caller should delete
        // the result.
        Compaction *CompactRange(unsigned level, const InternalKey *begin, const InternalKey *end);
        ...
        bool NeedsCompaction(bool *levels, bool seek_driven) const
        {
            ...
            return PickCompactionLevel(levels, seek_driven, &is_rewrite, &guard_seek_compaction) != config::kNumLevels;
        }
        ...
        InternalKeyComparator GetInternalComparator()
        {
            return icmp_;
        }
      ...
      private:
        ...
        void GetCompactionBoundaries(Version *version, unsigned level, std::vector<FileMetaData *> *LA,
      ...
      private:
        ...
        Slice compact_pointer_guard_[config::kNumLevels];
    ...
    class Compaction
    {
      public:
        ...
        size_t num_input_files(int which) const
        {
        ...
        uint64_t MinOutputFileSize() const
        {
            return min_output_file_size_;
        }
        ...
        // higher level?
        bool CrossesBoundary(const ParsedInternalKey &old_key, const ParsedInternalKey &new_key, size_t *hint) const;
