...
#    define SHARED_PTR std::tr1::shared_ptr
    ...
    class DBImpl : public DB
    {
      public:
        ...
        virtual Status GetReplayIterator(const std::string &timestamp, ReplayIterator **iter);
        ...
        virtual Status PrintFilesSizeByGuard();
      ...
      private:
        ...
        Iterator *NewInternalIterator(const ReadOptions &, uint64_t number, SequenceNumber *latest_snapshot,
        ...
        Status WriteLevel0TableGuards(MemTable *mem, VersionEdit *edit, Version *base, std::vector<uint64_t> &numbers,
                                      FileLevelFilterBuilder *file_level_filter_builder,
                                      std::vector<std::string *> *file_level_filters) EXCLUSIVE_LOCKS_REQUIRED(mutex_);

        Status SequenceWriteBegin(Writer *w, WriteBatch *updates) EXCLUSIVE_LOCKS_REQUIRED(mutex_);
        ...
        Status BackgroundCompactionGuards(FileLevelFilterBuilder *file_level_filter_builder)
            EXCLUSIVE_LOCKS_REQUIRED(mutex_);
        ...
        Status DoCompactionWorkForGuardsInALevel(CompactionState *compact) EXCLUSIVE_LOCKS_REQUIRED(mutex_);
        ...
        Status InstallCompactionResults(CompactionState *compact, const int level_to_add_new_files,
                                        std::vector<uint64_t> file_numbers,
                                        std::vector<std::string *> file_level_filters) EXCLUSIVE_LOCKS_REQUIRED(mutex_);
        ...
        struct ManualCompaction {
            ManualCompaction() : level(), done(), begin(), end(), tmp_storage()
            {
            }
            unsigned level;
            bool done;
