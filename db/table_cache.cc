...
namespace leveldb {
    ...
    {
        for (std::map<uint64_t, FileMetaData *>::iterator it = file_metadata_map.begin(); it != file_metadata_map.end();
             ++it) {
    ...
    Status TableCache::FindTable(uint64_t file_number, uint64_t file_size, Cache::Handle **handle, Timer *timer,
                                 const char *buf_file, bool fill_gd_cache)
    {
        Status s;
        ...
        *handle = cache_->Lookup(key);
        if (*handle == NULL) {
            ...
            s = env_->NewRandomAccessFile(fname, &file, buf_file, fill_gd_cache);
    ...
    void TableCache::SetFileMetaDataMap(uint64_t file_number, uint64_t file_size, InternalKey smallest,
                                        InternalKey largest)
    {
        ...
        ////		  printf("DEBUG :: Creating new FileMetaData for file %llu to set file metadata map in
        /// table_cache\n", file_number);
    ...
    Iterator *TableCache::NewIterator(const ReadOptions &options, uint64_t file_number, uint64_t file_size,
                                      Table **tableptr, const char *buf_file)
    {
        if (tableptr != NULL) {
            *tableptr = NULL;
        }
        ...
        Status s = FindTable(file_number, file_size, &handle, nullptr, buf_file, options.fill_geardb_cache);
        if (!s.ok()) {
    ...
    Status TableCache::Get(const ReadOptions &options, uint64_t file_number, uint64_t file_size, const Slice &k,
                           void *arg, void (*saver)(void *, const Slice &, const Slice &), Timer *timer)
    {
        //  printf("Tablecache Get().\n");
        Cache::Handle *handle = NULL;
        Status s;
        start_timer(GET_TABLE_CACHE_FIND_TABLE);
        s = FindTable(file_number, file_size, &handle, timer, nullptr, false);
        // MyLog("After finding table. \n");
        ...
        if (s.ok()) {
            ...
            // MyLog("Calling table->InternalGet. \n");
            s = t->InternalGet(options, k, arg, saver, timer);
            // MyLog("Releasing cache->handle.\n");
