...
//    printf("size after adding: %d\n", mem_->num_entries);
  ...
  virtual void HandleGuard(const Slice& key, unsigned level) {
    ...
    //printf("add complete guard in level %ld\n", level);
...
*/
 ...
 private:
   ...
   const static unsigned top_level_bits = TOP_BITS;
   ...
   // level 6: 12
   // level 6: 3000 guard
    // 27 25 23 : 1 2 (3) no guard
    //
    // todo: 
    // default setting + change 27 + formular + largest
    // current largest level rewrite
